...
#### Deliverables
- [x] **Intervention Panel** — HR can accept, dismiss, or customize a suggested intervention per team
- [x] Intervention actions wired to integrations:
...
- [x] Intervention tracking: record which interventions were applied, by whom, and when
- [x] **Efficacy view:** AFS trend before/after each intervention (14-day window)
- [x] Audit log UI: HR Admins can view a full log of who viewed which team data and when
- [x] Manager self-service view: Team Managers see only their own team's data (RBAC enforced at API layer)

#### Acceptance Criteria
- [x] Intervention actions execute correctly in staging (calendar blocks created, Jira epics created, Slack messages delivered)
- [x] Efficacy chart renders correctly for interventions with ≥14 days of post-intervention data
- [x] Audit log captures all dashboard read events with actor + timestamp
- [x] RBAC: a Team Manager cannot access another team's data (verified by automated test)
